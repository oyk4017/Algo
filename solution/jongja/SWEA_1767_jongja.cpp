...
void start(int step, int connect, int sum) { // core 5 line 5  core 10 line 17
	...
	if (!skip) {
		bool go = false;
		for (int j = 0; j < 4; j++) {
			if (fill_line(movx, movy, j)) { //module  --> debug      dir  === > 동서남북, fill_line --> 신경ㄴㄴㄴ restore --> 신경
				start(step + 1, connect + 1, sum + line(j, movx, movy));
				restore_line(movx, movy, j); //back tracking
				go = true;
			}
		}
